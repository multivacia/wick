...
PREVIOUS_HOST_STRATEGY = VPS
PREVIOUS_HOST_PROVIDER = HostGator
PREVIOUS_HOST_ID = wick-r3e-collector-01
PREVIOUS_DURABLE_STORE_PATH = /srv/wick
HOST_PROVIDER_INSTANCE_ID = DEFERRED_WITH_HOSTGATOR
HOST_PUBLIC_IP = DEFERRED_WITH_HOSTGATOR
HOSTNAME_CONFIRMED = DEFERRED_WITH_HOSTGATOR
AUTOMATION_COMMAND = python -m wick.r3e.future_unseen run-cycle
SCHEDULER_IMPLEMENTED = local_systemd_cron_or_windows_task_prepared_disabled
...
DECISION = CHANGE_HOST_STRATEGY_TO_LOCAL_PERSISTENT_HOST
RECOMMENDED_DECISION = APPROVE_LOCAL_PREPARATION_BLOCK_ACTIVATION
STRATEGY_UPDATED_AT = 2026-07-19T00:57:10Z
```
...
```text
RECOMMENDED_HOST_STRATEGY = LOCAL_PERSISTENT_HOST
HOST_PROVIDER = LOCAL
HOST_ID = wick-r3e-local-collector-01
DURABLE_STORE_PATH = $HOME/wick-r3e
HOSTGATOR_VPS_STATUS = DEFERRED_FUTURE_MIGRATION
STORE_OWNERSHIP = durable_local_home_not_ephemeral_not_github_actions
APP_CHECKOUT_PATH = $HOME/wick-r3e/app
DATA_SYMLINK_STRATEGY = app/data/future_unseen->$HOME/wick-r3e/data/future_unseen; app/reports/r3e_future_unseen->$HOME/wick-r3e/reports/r3e_future_unseen
SCHEDULER_STRATEGY = local_systemd_user_timer_or_cron_or_windows_task_hourly_minute_15_UTC_PREPARED_DISABLED
SECRET_STRATEGY = LOCAL_PROTECTED_ENV_FILE ($HOME/wick-r3e/config/r3e-collector.env mode 0600)
OBSERVABILITY_STRATEGY = local_logs_plus_automation_state_email_deferred
ROLLBACK_STRATEGY = disable_local_scheduler_keep_append_only_store_restore_from_$HOME/wick-r3e/backups
OPERATIONAL_OWNER = Gustavo Almeida
RECOMMENDED_DECISION = APPROVE_LOCAL_PREPARATION_BLOCK_ACTIVATION
IMPACT_ASSESSMENT_STATUS = APPROVED
IMPLEMENTATION_AUTHORIZED = true
SCHEDULER_ACTIVATION_AUTHORIZED = false
NEXT_ACTION = PREPARE_LOCAL_HOST_ARTIFACTS_THEN_AWAIT_ACTIVATION_AUTH
```

Histórico HostGator preservado (`PREVIOUS_*` / ficha VPS `DEFERRED_FUTURE_MIGRATION`). A ativação real do scheduler local depende de autorização humana posterior; e-mail permanece `DEFERRED`.
