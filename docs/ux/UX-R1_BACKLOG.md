...
### UX-B3 — Operational MVP Screen Contracts (+ future Overview prototype)

```text
TASK_ID = OPERATIONAL-MVP-SCREEN-CONTRACTS-001
STATUS = SPECIFICATION_READY_FOR_HUMAN_REVIEW
PRIORITY = P0
MVP = REQUIRED
DEPENDS_ON = UX-B1
PARALLEL_WITH = UX-B2, UX-B4
...
Entregáveis (esta fase — contratos, sem UI):

- screen contracts (Visão Geral, Execuções, Readiness, Host e Scheduler);
- data contract catalog;
- state matrix;
- safe fixture catalog;
- impact assessment + independent review;
- read-only adapter recommendation.

Entregáveis futuros (somente com `UX_B3_IMPLEMENTATION_AUTHORIZED=true`):
...
UX_B2_AUTHORIZED_INCREMENT = I1
I1_IMPLEMENTATION_STATUS = BLOCKED_PENDING_SEPARATE_TASK_AND_HUMAN_AUTHORIZATION
UX_B3_STATUS = SPECIFICATION_READY_FOR_HUMAN_REVIEW
UX_B4_STATUS = MERGED
