...
const screenRoot = join(testDir, "../../../src/screens/collected-data");
const viewmodelFiles = [
  join(testDir, "../../../src/viewmodels/buildCollectionDataQualityViewModel.ts"),
  join(testDir, "../../../src/viewmodels/filterCollectionDataQuality.ts"),
  join(testDir, "../../../src/viewmodels/collectionDataQualityEnums.ts"),
  join(testDir, "../../../src/viewmodels/collectionDataQualityTypes.ts"),
];
const fixtureFile = join(
  testDir,
  "../../../src/fixtures/collectionDataQuality.ts",
);
...
describe("Collected Data architecture boundaries", () => {
  it("does not import network, filesystem, markdown, env, or ops controls", () => {
    const files = [...listFiles(screenRoot), ...viewmodelFiles, fixtureFile];
  ...
  it("does not offer download or external href generation", () => {
    const files = listFiles(screenRoot);
    const joined = files.map((f) => readFileSync(f, "utf8")).join("\n");
